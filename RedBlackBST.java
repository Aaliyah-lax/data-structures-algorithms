...
public class RedBlackBST<Key extends Comparable<Key>, Value>
{
     ...
     * <em>maximum</em>, <em>floor</em>, and <em>ceiling</em>. It also provides
     * a <em>keys</em> method for iterating over all of the keys. A symbol table
     * implements the <em>associative array</em> abstraction: when associating a
     * value with a key that is already in the symbol table, the convention is
     * to replace the old value with the new value. Unlike
     * {@link java.util.Map}, this class uses the convention that values cannot
     * be {@code null}—setting the value associated with a key to {@code null}
     ...
     * and calls the {@code compareTo()} and method to compare two keys. It does
     * not call either {@code equals()} or {@code hashCode()}.
     * <p>
     * This implementation uses a <em>left-leaning red-black BST</em>. The
     * <em>put</em>, <em>get</em>, <em>contains</em>, <em>remove</em>,
     * <em>minimum</em>, <em>maximum</em>, <em>ceiling</em>, <em>floor</em>,
     * <em>rank</em>, and <em>select</em> operations each take &Theta;(log
     * <em>n</em>) time in the worst case, where <em>n</em> is the number of
     * key-value pairs in the symbol table. The <em>size</em>, and
     * <em>is-empty</em> operations take &Theta;(1) time. The <em>keys</em>
     * methods take
     * <em>O</em>(log <em>n</em> + <em>m</em>) time, where <em>m</em> is the
     * number of keys returned by the iterator. Construction takes &Theta;(1)
     * time.
     * <p>
     ...
     * {@link AVLTreeST}. For additional documentation, see
    ...
    public Key getLeftKey(Key key)
    {
        if (key == null)
            throw new IllegalArgumentException("argument to get() is null");
        return getLeftKey(root, key);
    }
        private Key getLeftKey(Node x, Key key)
    {
        while (x != null) {
            int cmp = key.compareTo(x.key);
            if (cmp < 0)
        ...
        public Key getrightKey(Key key)
    {
        if (key == null)
            throw new IllegalArgumentException("argument to get() is null");
        return getrightKey(root, key);
    }
        private Key getrightKey(Node x, Key key)
    {
        while (x != null) {
            int cmp = key.compareTo(x.key);
            if (cmp < 0)
    ...
    private Node put(Node h, Key key, Value val)
    {
        ...
        if (isRed(h.left) && isRed(h.right)) {
            System.out.println("Flip color at " + h.key);
            flipColors(h);
        }
    ...
    /**
     * Return the key in the symbol table of a given {@code rank}. This key has
     * the property that there are {@code rank} keys in the symbol table that
     * are smaller. In other words, this key is the ({@code rank}+1)st smallest
    ...
    public static void main(String[] args)
    {
        ...
        int[] temp  = {1, 6, 8, 5, 0, 2};
//        for(int i = 0; i < temp.length; i++)
//        {
//            temp[i] = rand.nextInt(20);
...
//        }
    ...
    public static void show(RedBlackBST<Integer, Integer> st)
    {

        for (Integer s : st.keys()) {
            System.out.print(s + " " );
            System.out.print("left " + " " + st.getLeftKey(s));
            System.out.print("           right  "  + " " + st.getrightKey(s));
            if (st.getColor(s))
